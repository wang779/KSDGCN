...
class DatesetReader:
    ...
    def __read_data__(fname, tokenizer, max_seq_len=-1):
        ...
        fin.close()
        # 情感图
        fin = open(fname + p.know_file[0].split('_')[2].split('.')[0] + '.sentic', 'rb')
        ...
        for i in range(0, len(lines), 2):
            ...
            # 将文本（context）转换为对应的索引序列（context_indices）
            ...
            data = {
                'context': context,  # 句子文本内容
                'context_indices': context_indices,  # 句子单词id
                'dependency_graph': dependency_graph,  # 依赖图
                'label': label,  # 标签
                'sentic_graph': sentic_graph,  # 情感图
            }
