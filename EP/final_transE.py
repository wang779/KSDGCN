...
entities2id = []
relations2id = []
# 生成知识图谱中所有实体的嵌入向量，存储在dic_train_ent_emb中
...
class TransE:
    ...
    def training_run(self, epochs=10, nbatches=100, out_file_title=''):
        # 对实体的嵌入向量进行归一化处理。
        # 对于每个批次，随机抽取一部分三元组，构成一个批次样本 Tbatch。
        # 对于每个样本，随机选择一个头实体或尾实体，并用其他实体替换，构成一个损坏的样本。
        # 将原始样本和损坏样本构成的批次输入到 update_triple_embedding 方法中进行参数更新。
        # 训练结束后，将实体的嵌入向量存储到字典 dic 中并返回。
        ...
        for e in self.entities.keys():
            # f1.write("\t")
            # f1.write(e + "\t")
            dic[e] = self.entities[e]
            # f1.write(e + "\t" + str(list(self.entities[e])))
            # f1.write("\n")
...
file1 = p.dataset_path + "/final_test_triple.txt"
file2 = p.dataset_path + "/final_entity_test.txt"
file3 = p.dataset_path + "/final_relation_test.txt"

entity_set, relation_set, triple_list = dataloader(file1, file2, file3)

# modify by yourself
transE = TransE(entity_set, relation_set, triple_list, embedding_dim=150, lr=0.01, margin=1.0, norm=2)
transE.data_initialise()
dic_test_ent_emb = transE.training_run(out_file_title="test_")

file4 = p.dataset_path + "/final_train_triple.txt"
file5 = p.dataset_path + "/final_entity_train.txt"
file6 = p.dataset_path + "/final_relation_train.txt"

entity_set1, relation_set1, triple_list1 = dataloader(file4, file5, file6)


transE1 = TransE(entity_set1, relation_set1, triple_list1, embedding_dim=150, lr=0.01, margin=1.0, norm=2)
transE1.data_initialise()
dic_train_ent_emb = transE1.training_run(out_file_title="train_")
print("===========================================================================")
